...
public class KDTree<T> {
    ...
    *   </PRE>
     ...
     */
    public void replaceInsert(double[] key,T value)
    throws KeySizeException, KeyDuplicateException{
   ...
   * public List<T> nearestHamming(double [] key, double dist) 
    ...
    */
    ...
    public ArrayList<T> toArrayListValue(){
    	return m_root.toArrayListValue(0);
    }
    ...
    Checker<T> checker) throws KeySizeException {
    	...
    	return getnbrs(key,n,Double.MAX_VALUE,checker);
     ...
     * @throws KeySizeException
     */
	private NearestNeighborList<KDNode<T>> getnbrs(double[] key, double max_dist)
			throws KeySizeException {
		return getnbrs(key, m_count, max_dist, null);
	}
     ...
     */
	private NearestNeighborList<KDNode<T>> getnbrs(double[] key, int n,
			double max_dist, Checker<T> checker) throws KeySizeException {
	...
	/**
	 * modify by zjb, but hamming distance is not right for this method
	 * @param key
	 * @param dist
	 * @param metric
	 * @return
	 ...
	 */
	 private  List<T> nearestDistance(double [] key, double dist, 
				...
				NearestNeighborList<KDNode<T>> nnl = getnbrs(key,dist);	    
				int n = nnl.getSize();
				Stack<T> nbrs = new Stack<T>();
    ...
    /* modify by zjb, this method is not good, it need to scan the whole database
    private  List<T> nearestDistance(double [] key, double dist, 
    DistanceMetric metric) throws KeySizeException {
