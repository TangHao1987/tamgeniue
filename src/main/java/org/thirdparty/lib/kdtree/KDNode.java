...
class KDNode<T> {
	...
	synchronized (t) {
            if (key.equals(t.k)) {
                ...
                if (t.deleted == was_deleted) {
                    ...
                    return 1;//modified by zjb, correct debug of k-d tree, this should be 1, (previous is -1)
                }
                return -1;//modified by zjb, this should be -1, (previous is 1)
   ...
   protected static <T> void nnbr(KDNode<T> kd, HPoint target, HRect hr,
     ...
     * @return
     */
    protected ArrayList<T> toArrayListValue(int depth){
    	ArrayList<T> res=new ArrayList<T>();
    	
    	if(null!=v){
    	if(null!=left){
    	ArrayList<T> l=left.toArrayListValue(depth+1);
    	if(null!=l) res.addAll(l);
    	}
    	if(!deleted){
        	res.add(v);
        }
    	if(null!=right){
    	ArrayList<T> r=right.toArrayListValue(depth+1);
    	if(null!=r) res.addAll(r);
    	}
    	}
