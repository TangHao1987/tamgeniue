...
package org.thirdparty.lib.storagemanager;
...
public class RandomEvictionsBuffer extends Buffer
{
	...
	{
		assert memoryBuffer.size() <= m_capacity;

		if (memoryBuffer.size() == m_capacity) removeEntry();
		memoryBuffer.put(id, e);
	}
	...
	void removeEntry()
	{
		...
		int entry = m_random.nextInt(memoryBuffer.size());

		Iterator it = memoryBuffer.entrySet().iterator();
		...
		memoryBuffer.remove(new Integer(id));
	}
